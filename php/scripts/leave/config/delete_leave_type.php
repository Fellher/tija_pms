<?php
/**
 * Delete Leave Type/Policy (Soft Delete)
 * Handles deletion of leave types and policies
 */
...
try {
    ...
    if ($applications && count($applications) > 0) {
        throw new Exception('Cannot delete leave type. It is currently being used in ' . count($applications) . ' leave application(s).');
    }
    ...
    if ($entitlements && count($entitlements) > 0) {
        throw new Exception('Cannot delete leave type. It has ' . count($entitlements) . ' active entitlement(s).');
    }
    ...
    if ($DBConn->update_table('tija_leave_types', $updateData, array('leaveTypeID' => $leaveTypeID))) {
        ...
        $response['data'] = array('leaveTypeID' => $leaveTypeID);
    } else {
