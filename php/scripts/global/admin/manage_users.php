...
if ( $isValidUser) {
  ...
  var_dump($_FILES);
    ...
    $departmentID = (isset($_POST['departmentID']) && !empty($_POST['departmentID'])) ? Utility::clean_string($_POST['departmentID']) : "";

    // Allowances
    $housingAllowance = (isset($_POST['housingAllowance']) && !empty($_POST['housingAllowance'])) ? Utility::clean_string($_POST['housingAllowance']) : "";
    $transportAllowance = (isset($_POST['transportAllowance']) && !empty($_POST['transportAllowance'])) ? Utility::clean_string($_POST['transportAllowance']) : "";
    $medicalAllowance = (isset($_POST['medicalAllowance']) && !empty($_POST['medicalAllowance'])) ? Utility::clean_string($_POST['medicalAllowance']) : "";
    $communicationAllowance = (isset($_POST['communicationAllowance']) && !empty($_POST['communicationAllowance'])) ? Utility::clean_string($_POST['communicationAllowance']) : "";
    $otherAllowances = (isset($_POST['otherAllowances']) && !empty($_POST['otherAllowances'])) ? Utility::clean_string($_POST['otherAllowances']) : "";
    ...
    $commissionRate = (isset($_POST['commissionRate']) && !empty($_POST['commissionRate'])) ? Utility::clean_string($_POST['commissionRate']) : "";
    ...
    $supervisorID = (isset($_POST['supervisorID']) && !empty($_POST['supervisorID'])) ? Utility::clean_string($_POST['supervisorID']) : "";
    ...
    if(!$userID) {
      ...
      if($employeeDetails && count($errors) == 0) {
        // Use existing values for fields not provided in the form
        if(!$organisationID) $organisationID = $employeeDetails->orgDataID;
        if(!$entityID) $entityID = $employeeDetails->entityID;

        // check and populate changes - only update fields that are actually provided
        if($firstName && $employeeDetails->FirstName != $firstName) $changes['FirstName'] = $firstName;
        if($surname && $employeeDetails->Surname != $surname) $changes['Surname'] = $surname;
        if($email && $employeeDetails->Email != $email) $changes['Email'] = $email;
        ...
        // check and update the users employment details - only update fields that are provided
        if($prefixID && $prefixID != $employeeDetails->prefixID)  $employeeChanges['prefixID'] = $prefixID;
        if($organisationID && $employeeDetails->orgDataID != $organisationID) $employeeChanges['orgDataID'] = $organisationID;
        if($entityID && $employeeDetails->entityID != $entityID) $employeeChanges['entityID'] = $entityID;
        if($jobTitleID && $employeeDetails->jobTitleID != $jobTitleID) $employeeChanges['jobTitleID'] = $jobTitleID;
        if($employeeTypeID && $employeeDetails->employmentStatusID != $employeeTypeID) $employeeChanges['employmentStatusID'] = $employeeTypeID;
        if($nationalID && $employeeDetails->nationalID != (int)$nationalID) $employeeChanges['nationalID'] = (int)$nationalID;
        ...
        $supervisorID && $employeeDetails->supervisorID != $supervisorID ? $employeeChanges['supervisorID'] = $supervisorID : "";


        var_dump($employeeDetails->nationalID);
        var_dump($nationalID);
        if($nhifNumber && $employeeDetails->nhifNumber != $nhifNumber) $employeeChanges['nhifNumber'] = $nhifNumber;
        if($nssfNumber && $employeeDetails->nssfNumber != $nssfNumber) $employeeChanges['nssfNumber'] = $nssfNumber;
        if($pin && $employeeDetails->pin != $pin) $employeeChanges['pin'] = $pin;

        if($basicSalary && $employeeDetails->basicSalary != (float)$basicSalary) $employeeChanges['basicSalary'] = (float)$basicSalary;
        ...
        if($overtimeAllowed && $employeeDetails->overtimeAllowed != $overtimeAllowed) $employeeChanges['overtimeAllowed'] = $overtimeAllowed;
        if($departmentID && $employeeDetails->departmentID != $departmentID) $employeeChanges['departmentID'] = $departmentID;

        // Bonus & Commission (now in user_details table) - only update if explicitly provided
        // Note: These fields use isset() check on POST data to determine if they were submitted
        if(isset($_POST['bonusEligible'])) {
        ...
        if(isset($_POST['commissionEligible'])) {
        ...
        if(isset($employeeDetails->commissionRate) && $commissionRate !== "" && $employeeDetails->commissionRate != (float)$commissionRate) $employeeChanges['commissionRate'] = (float)$commissionRate;
        if(!isset($employeeDetails->commissionRate) && $commissionRate !== "") $employeeChanges['commissionRate'] = (float)$commissionRate;
        if($dailyWorkHours && $employeeDetails->dailyHours != $dailyWorkHours) $employeeChanges['dailyHours'] = $dailyWorkHours;
        if($weekWorkDays && $employeeDetails->weekWorkDays != $weekWorkDays) $employeeChanges['weekWorkDays'] = $weekWorkDays;
        if($dateOfEmployment && $employeeDetails->employmentStartDate != $dateOfEmployment) $employeeChanges['employmentStartDate'] = $dateOfEmployment;
        if($dateOfTermination && $employeeDetails->employmentEndDate != $dateOfTermination) $employeeChanges['employmentEndDate'] = $dateOfTermination;
        ...
        if($workHourRounding && $employeeDetails->workHourRoundingID != $workHourRounding) $employeeChanges['workHourRoundingID'] = $workHourRounding;
        ...
        // Only process if at least one allowance field was submitted in the form
        if(isset($_POST['housingAllowance']) || isset($_POST['transportAllowance']) ||
           isset($_POST['medicalAllowance']) || isset($_POST['communicationAllowance']) ||
           isset($_POST['otherAllowances'])) {
          ...
          $allowanceData = [];
          if(isset($_POST['housingAllowance'])) $allowanceData['housingAllowance'] = (float)$housingAllowance;
          if(isset($_POST['transportAllowance'])) $allowanceData['transportAllowance'] = (float)$transportAllowance;
          if(isset($_POST['medicalAllowance'])) $allowanceData['medicalAllowance'] = (float)$medicalAllowance;
          if(isset($_POST['communicationAllowance'])) $allowanceData['communicationAllowance'] = (float)$communicationAllowance;
          if(isset($_POST['otherAllowances'])) $allowanceData['otherAllowances'] = (float)$otherAllowances;
