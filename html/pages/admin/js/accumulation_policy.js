...
class AccumulationPolicyManager {
    ...
    async handleFormSubmit(e) {
        ...
        try {
            const response = await fetch(siteUrl + 'php/scripts/admin/manage_accumulation_policy.php', {
                method: 'POST',
                body: formData
            });
    ...
    updateAccrualTypeHelp(accrualType) {
        const helpTexts = {
            'Monthly': 'Days accrued each month (e.g., 2.5 days per month)',
            'Quarterly': 'Days accrued every 3 months (e.g., 7.5 days per quarter)',
            'Annual': 'Days accrued once per year (e.g., 30 days per year)',
            'Continuous': 'Days accrued continuously (e.g., 0.1 days per day)',
            'Custom': 'Custom accrual schedule defined by rules'
        };
        ...
        if (!helpElement) {
            ...
            document.getElementById('accrualType').parentNode.appendChild(helpElement);
        }
