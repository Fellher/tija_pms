<?php 
/**
 * Client Management Dashboard - Optimized & Refactored Version
 * 
 * A comprehensive client management system built with modern web development best practices.
 * This module provides a complete solution for managing client relationships, tracking
 * activities, and maintaining data integrity within the TIJA PMS ecosystem.
 * 
 * ARCHITECTURE OVERVIEW:
 * =====================
 * 1. Configuration Layer    - Centralized settings and permissions
 * 2. Data Layer           - Client data retrieval and processing
 ...
 * • Responsive Data Table          - Custom pagination with performance optimization
 * • Client Activity Tracking       - Real-time KPIs and metrics
 * • Export Capabilities           - CSV export with filtered data support
 * • Modal Management System       - Add/Edit/Upload client operations
 ...
 * @todo       - Implement real-time notifications for client updates
 * @todo       - Add client activity timeline view
 * @todo       - Implement bulk operations for client management
 */
 ...
 * 
 * Centralized configuration for the client management module.
 * All settings are organized by functional area for easy maintenance.
 * 
 * @var array $config['client'] - Main configuration array
 */
$config['client'] = [
    ...
    // ========================================================================
    'modals' => [
        'size' => 'modal-md',                  // Default modal size
    ...
    // ========================================================================
    'kpis' => [
        'enabled' => true,                     // Enable KPI dashboard
        'refreshInterval' => 30000,            // Auto-refresh interval (30 seconds)
        'showActivityMetrics' => true,         // Show client activity metrics
        'showFinancialMetrics' => false,       // Show financial metrics (future)
    ...
    'export' => [
        ...
        'fileName' => 'clients_export_' . date('Y-m-d_H-i-s'), // Default filename
 ...
 * 
 * Defines user permissions for various client management operations.
...
$permissions = [
    ...
    'canAddClients' => $isValidUser ?? false,      // Add new clients
    'canEditClients' => $isValidUser ?? false,     // Edit existing clients
    'canDeleteClients' => $isAdmin ?? false,       // Delete clients (admin only)
    'canViewClients' => $isValidUser ?? false,     // View client details
    
    // Advanced operations
    'canBulkUpload' => $isAdmin ?? false,          // Bulk upload clients
    'canExportData' => $isValidUser ?? false,      // Export client data
    'canViewSalaries' => $isValidAdmin ?? false,   // View salary information
    'canManagePermissions' => $isAdmin ?? false,   // Manage user permissions
    
    // Administrative operations
    'canViewReports' => $isValidUser ?? false,     // View client reports
 ...
 * =======================
 * Functions for managing client status indicators and visual elements.
 */

/**
 * Generate client status badge HTML
 * 
 * Creates a Bootstrap badge element indicating whether a client is in-house
 * or external. The badge styling provides immediate visual feedback about
 * the client type.
 * 
 * @param object $client - Client object containing status information
 * @return string - HTML badge element with appropriate styling
 * 
 * @example
 * $badge = generateClientStatusBadge($client);
 * echo $badge; // Outputs: <span class="badge rounded-pill bg-danger-transparent">In House</span>
 * 
 * @since 3.0.0
 * @version 1.0.0
 */
function generateClientStatusBadge($client) {
    ...
    if (!is_object($client)) {
        error_log('ClientStatusBadge: Invalid client object provided');
        return '<span class="badge rounded-pill bg-secondary-transparent">Unknown</span>';
    }
    
    // Extract status with fallback to external
    $status = $client->inHouse ?? 'N';
    
    // Generate appropriate badge based on status
    switch($status) {
        case 'Y':
            return '<span class="badge rounded-pill bg-danger-transparent" title="Internal client">In House</span>';
        case 'N':
        default:
            return '<span class="badge rounded-pill bg-success-transparent" title="External client">External</span>';
    }
}

/**
 * CLIENT ACTIVITY CALCULATION
 * ===========================
 * Functions for calculating and managing client activity metrics.
 */

/**
 * Calculate client activity metrics
 * 
 * Analyzes a client's current activity by counting open sales cases and projects.
 * This function provides real-time insights into client engagement and workload.
 * 
 * @param object $client - Client object containing clientID
 * @param string $orgDataID - Organization data identifier
 * @param string $entityID - Entity identifier
 * @param object $DBConn - Database connection object
 * @return array - Activity metrics with detailed breakdown
 * 
 * @return array {
 *     @var int $sales - Number of open sales cases
 *     @var int $projects - Number of open projects
 *     @var int $total - Total open cases (sales + projects)
 *     @var string $breakdown - Human-readable activity summary
 * }
 * 
 * @example
 * $activity = calculateClientActivity($client, $orgDataID, $entityID, $DBConn);
 * echo $activity['total']; // Outputs: 5
 * echo $activity['breakdown']; // Outputs: "5 (3 Sales, 2 Projects)"
 * 
 * @since 3.0.0
 * @version 1.0.0
 * 
...
function calculateClientActivity($client, $orgDataID, $entityID, $DBConn) {
    // Input validation
    if (!is_object($client) || !isset($client->clientID)) {
        error_log('ClientActivity: Invalid client object provided');
        return [
            'sales' => 0,
            'projects' => 0,
            'total' => 0,
    ...
    if (!$DBConn) {
        error_log('ClientActivity: Database connection not available');
        throw new Exception('Database connection required for activity calculation');
    }
    
    try {
        // Count open sales cases for this client
        $openSalesCases = Sales::sales_cases([
    ...
    $openCasesSales = $openSalesCases ? count($openSalesCases) : 0;
        ...
        // Count open projects for this client
        $openProjects = Projects::projects_full([
        'clientID' => $client->clientID, 
        'orgDataID' => $orgDataID, 
        'entityID' => $entityID, 
        'projectStatus' => 'open'
    ...
    $totalOpenCases = $openCasesSales + $openCasesProjects;
        ...
        $breakdown = $totalOpenCases > 0 
            ? "{$totalOpenCases} ({$openCasesSales} Sales, {$openCasesProjects} Projects)"
            : "0";
    ...
    return [
        ...
        error_log('ClientActivity: Error calculating activity - ' . $e->getMessage());
        return [
            'sales' => 0,
            'projects' => 0,
            'total' => 0,
 ...
 * ===========================
 * Functions for generating URLs and formatting display data.
 */

/**
 * Generate client profile link
 * 
 * Creates a properly formatted URL for accessing a client's detailed profile page.
 * The URL includes necessary parameters for navigation and context preservation.
 * 
 * @param object $client - Client object containing clientID
 * @param string $base - Base URL for the application
 * @param array $params - URL parameters (s, ss, etc.)
 * @return string - Complete client profile URL
 * 
 * @example
 * $link = generateClientProfileLink($client, $base, ['s' => 'user', 'ss' => 'clients']);
 * // Outputs: "../html/?s=user&ss=clients&p=client_details&client_id=123"
 * 
 * @since 3.0.0
 * @version 1.0.0
 */
function generateClientProfileLink($client, $base, $params) {
    ...
    if (!is_object($client) || !isset($client->clientID)) {
        error_log('ClientProfileLink: Invalid client object provided');
        return '#';
    }
    
    if (!is_array($params) || !isset($params['s']) || !isset($params['ss'])) {
        error_log('ClientProfileLink: Invalid parameters provided');
        return '#';
    }
    
    // Sanitize client ID to prevent XSS
    $clientID = htmlspecialchars($client->clientID, ENT_QUOTES, 'UTF-8');
    $section = htmlspecialchars($params['s'], ENT_QUOTES, 'UTF-8');
    $subsection = htmlspecialchars($params['ss'], ENT_QUOTES, 'UTF-8');
    
    return "{$base}html/?s={$section}&ss={$subsection}&p=client_details&client_id={$clientID}";
}

/**
 * Format client address for display
 * 
 * Combines address components into a single, readable string for display.
 * Handles missing data gracefully and ensures proper formatting.
 * 
 * @param object|null $addressDetails - Address details object
 * @param object|null $country - Country object
 * @return string - Formatted address string
 * 
 * @example
 * $address = formatClientAddress($addressDetails, $country);
 * // Outputs: "123 Main St Nairobi Kenya"
 * 
 * @since 3.0.0
 * @version 1.0.0
 */
function formatClientAddress($addressDetails, $country) {
    // Handle missing address details
    if (!$addressDetails) {
        return "Not Set";
    }
    
    // Extract and sanitize address components
    $address = isset($addressDetails->address) 
        ? Utility::clean_string($addressDetails->address) 
        : '';
    
    $city = isset($addressDetails->city) 
        ? $addressDetails->city 
        : '';
    
    $countryName = (isset($country->countryName)) 
        ? $country->countryName 
        : "";
    
    // Combine components and clean up
    $formattedAddress = trim("{$address} {$city} {$countryName}");
    
    // Return formatted address or fallback
    return !empty($formattedAddress) ? $formattedAddress : "Not Set";
}
 ...
 * CLIENT DATA LOADING
 * ===================
 * Loads client data and related information for the current organizational context.
 ...
 * ACTIVITY ANALYSIS
 * ================
 * Analyzes client activity by examining sales cases and projects.
 * This provides insights into client engagement and workload distribution.
 */

// Retrieve sales cases for the current organization and entity
$salesCases = Sales::sales_cases([
    'orgDataID' => $orgDataID, 
    'entityID' => $entityID
], false, $DBConn);
...
<script>
/**
 * CLIENT MANAGEMENT INTERACTION LAYER
 * ===================================
 * 
 * This JavaScript module provides comprehensive client management functionality
 ...
 * 3. Modal Management     - Client add/edit/upload operations
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const CONFIG = {
        // Table configuration
        itemsPerPage: <?= $config['client']['display']['itemsPerPage'] ?>,
        maxVisiblePages: <?= $config['client']['table']['pagination']['maxVisiblePages'] ?>,
        debounceDelay: <?= $config['client']['performance']['debounceSearch'] ?>,
        
        // Feature toggles
        enableSearch: <?= $config['client']['features']['enableSearch'] ? 'true' : 'false' ?>,
        enableAdvancedFiltering: <?= $config['client']['features']['enableAdvancedFiltering'] ? 'true' : 'false' ?>,
        enableExport: <?= $config['client']['features']['enableExport'] ? 'true' : 'false' ?>,
        
        // Performance settings
        enableLazyLoading: <?= $config['client']['features']['enableLazyLoading'] ? 'true' : 'false' ?>,
        lazyLoadThreshold: <?= $config['client']['performance']['lazyLoadThreshold'] ?>,
        
        // Export settings
        exportFileName: '<?= $config['client']['export']['fileName'] ?>',
        maxExportRecords: <?= $config['client']['export']['maxRecords'] ?>,
    };
     ...
     */
    const SELECTORS = {
        table: '#clientsDataTable',
        tbody: '#clientsDataTable tbody',
     ...
     * UTILITY FUNCTIONS
     * ================
     * Common utility functions for error handling, validation, and UI operations.
     */
     ...
     * ====================
     * Initializes the custom table system with data loading and display setup.
     */
     ...
     * 
     * Sets up the initial table state and renders the first page of data.
     * This function is called once during page load to establish the table.
     * 
    ...
    function initializeTable() {
        try {
            ...
            console.log('Client Management Table initialized successfully');
        } catch (error) {
            console.error('Error initializing table:', error);
            showErrorMessage('Failed to initialize table. Please refresh the page.');
        }
    }
    ...
    function updateTableDisplay() {
        try {
            ...
            pageRows.forEach((row, index) => {
                ...
                clonedRow.setAttribute('data-client-id', row.getAttribute('data-client-id') || '');
    ...
    <?php if ($config['client']['enableSearch']): ?>
    // Enhanced search functionality for client names and account owners
